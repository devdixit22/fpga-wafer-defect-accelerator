...
class FPGAInference:
    def __init__(self):
        w_dir = ROOT / "weights"
        self.b1 = np.load(w_dir / "conv1_bias_int8.npy").astype(np.int32)
        self.b2 = np.load(w_dir / "conv2_bias_int8.npy").astype(np.int32)
        self.w3 = np.load(w_dir / "fc1_weight_int8.npy").astype(np.int32)
        self.b3 = np.load(w_dir / "fc1_bias_int8.npy").astype(np.int32)
        self.w4 = np.load(w_dir / "fc2_weight_int8.npy").astype(np.int32)
        self.b4 = np.load(w_dir / "fc2_bias_int8.npy").astype(np.int32)
    ...
    def forward(self, img):
        ...
        c1 = sim.run_accelerator(ROOT / "weights" / "conv1_weight_int8.npy", cols1.shape[0])
        ...
        # Testbench currently hardcoded to read conv1_weight_int8.mem, so we trick it
        shutil.copy(ROOT / "weights" / "conv2_weight_int8.mem", ROOT / "conv1_weight_int8.mem")
        
        c2 = sim.run_accelerator(ROOT / "weights" / "conv2_weight_int8.npy", cols2.shape[0])
        ...
        flat = c2.flatten().astype(np.int32)
